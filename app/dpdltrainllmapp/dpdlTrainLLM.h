...
#
# Dpdl (c)opyright 2023
# by SEE Solutions
#
# File: app/dpdltrainllmapp/dpdlTrainLLM.h
#
#
...
# main
println("testing LLM with Dpdl and C...")

dpdl_stack_var_put("data_model", "./DpdlLibs/llm/data/model")
dpdl_stack_var_put("data_source", "./DpdlLibs/llm/data/source")

dpdl_stack_push("dpdlbuf_res", "dpdl:applyvars", "dpdl:compile", "dpdl:-I./DpdlLibs/llm/llm_c")
>>c
#include <stdio.h>
#include <stdlib.h>
#include <ctype.h>
...
int dpdl_main(int argc, char **argv) {
    ...
    GPT2 model;
    gpt2_build_from_checkpoint(&model, "{{data_model}}/gpt2_124M.bin");

    // build the DataLoaders from tokens files. for now use tiny_shakespeare if available, else tiny_stories
    const char* tiny_stories_train = "{{data_source}}/TinyStories_train.bin";
    const char* tiny_stories_val = "{{data_source}}/TinyStories_val.bin";
    const char* tiny_shakespeare_train = "{{data_source}}/tiny_shakespeare_train.bin";
    const char* tiny_shakespeare_val = "{{data_source}}/tiny_shakespeare_val.bin";
    ...
    // build the Tokenizer
    Tokenizer tokenizer;
    tokenizer_init(&tokenizer, "{{data_model}}/gpt2_tokenizer.bin");
...
println("LLM embedded C exit code: " + exit_code)

raise(exit_code, "Error in executing llm training")
